...
const Login = props => {
  ...
  useEffect(() => {
    // For debugging, autologin to get in faster for testing, add a user and pword in the .env
    const { user } = sessionContext.state
    if (
      !user &&
      process.env.REACT_APP__DEBUG_AUTO_LOGIN_USER &&
      process.env.REACT_APP__DEBUG_AUTO_LOGIN_PWORD
    ) {
      faunaQueries
        .login(
          process.env.REACT_APP__DEBUG_AUTO_LOGIN_USER,
